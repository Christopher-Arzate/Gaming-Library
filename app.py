...
app.layout= dbc.Container([
    dbc.Row([
        dbc.Col(html.H1("Video Game Dashboard"), width=15,className="text-center my-5")
    ]),
    # General Statistics
    dbc.Row([
        dbc.Col(html.Div(f"Average Price of Games: {avg_game:,.2f}",className="text-center my-3"), width=5),
        dbc.Col(html.Div(f"Unique Number Games in my Library: {unique_game}",className="text-center my-3"),width=5)
    ],className="mb-5"),
    ...
    dbc.Row([
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    html.H4("Discounts by Video Game",className='Discount-title'),
                    dcc.Dropdown(
                        id="game-filter",
